...
export function DataTable({ columns, data, meta }) {
  ...
  const [form, setForm] = React.useState({ title: "", author: "", isbn: "", release_date: "" });
  const [editForm, setEditForm] = React.useState({ id: null, title: "", author: "", isbn: "", release_date: "" });
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <TableHead key={header.id} className="bg-emerald-600 text-white px-2 md:px-4 py-2">
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-48 text-center">
                  <div className="w-full flex justify-center items-center py-16">
                    ...
                    <span className="text-emerald-600 font-medium text-lg">Cargando libros...</span>
                  </div>
                </TableCell>
              </TableRow>
            ) : table.getRowModel().rows.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow key={row.id} className="even:bg-gray-50 odd:bg-white hover:bg-gray-100 transition-colors">
